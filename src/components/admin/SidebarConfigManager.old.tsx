...
export default function SidebarConfigManager() {
  ...
  const loadAllConfigs = useCallback(async () => {
    try {
      ...
      // Chỉ set selected config nếu chưa có config nào được chọn
      setSelectedConfig((prevSelected) => {
        if (!prevSelected && configs.length > 0) {
  ...
  useEffect(() => {
    if (permissions.canEditConfig && !hasLoadedConfigs) {
      loadAllConfigs();
    }
  }, [permissions.canEditConfig, hasLoadedConfigs, loadAllConfigs]);

  const handleSetDefault = async (configId: string) => {
    try {
      await setDefaultConfig(configId);
      setHasLoadedConfigs(false); // Force reload
