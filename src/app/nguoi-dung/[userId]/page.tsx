      ...
      try {
        ...
        if (data.success) {
          ...
          if (currentPage === 1) {
            ...
            const activePosts = newPosts.filter((p: UserPost) => p.status === "active");
            const sellPosts = activePosts.filter((p: UserPost) => p.type === "ban");
            const rentPosts = activePosts.filter((p: UserPost) => p.type === "cho-thue");

            setStats({
              totalPosts: totalItems,
              activePosts: totalItems, // All returned posts are active
        ...
        <div className="container mx-auto px-4 py-8 max-w-6xl">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            ...
            <div className="bg-white rounded-lg shadow-md p-4 text-center">
              ...
              <div className="text-sm text-gray-600">Bán</div>
            </div>
            <div className="bg-white rounded-lg shadow-md p-4 text-center">
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <div className="flex space-x-1 mb-6 bg-gray-100 p-1 rounded-lg">
              {[
                ...
                { key: "ban", label: "Bán", count: posts.filter(p => p.type === "ban").length },
                { key: "cho-thue", label: "Cho thuê", count: posts.filter(p => p.type === "cho-thue").length },
              ].map((tab) => (
                <button
                  key={tab.key}
              ...
              <>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {filteredPosts.map((post) => (
                    <div key={post._id} className="border border-gray-200 rounded-lg overflow-hidden hover:shadow-lg transition-shadow">
                      {/* Image */}
                      ...
                      <div className="p-4">
                        <Link
                          href={`/${post.type === "ban" ? "mua-ban" : "cho-thue"}/chi-tiet/${post.id}-${encodeURIComponent(post.title.toLowerCase().replace(/\s+/g, "-"))}`}
